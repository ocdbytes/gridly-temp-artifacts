...
// OpenZeppelin Contracts for Cairo v0.20.0-rc.0 (account/account.cairo)

// Modified Account Implementation :
//
// To be called by sequencer only
#[starknet::component]
pub mod BotComponent {
    ...
    > of InternalTrait<TContractState> {
        ...
        /// Sets sequencer public key
        fn _set_sequencer_key(ref self: ComponentState<TContractState>, sequencer_public_key: felt252) {
            self.Sequencer_public_key.write(sequencer_public_key);
            self.emit(OwnerAdded { new_owner_guid: sequencer_public_key });
        }

        /// Returns whether the given signature is valid for the given hash
        /// using the account's current public key.
        /// 
        /// Modified : Only sequencer can sign the transactions
        fn _is_valid_signature(
            self: @ComponentState<TContractState>, hash: felt252, signature: Span<felt252>,
        ) -> bool {
            let _public_key = self.Sequencer_public_key.read();
